...
export default function UserDetailContent({ user }: UserDetailContentProps) {
  ...
  const profileRadar: ProfileRadarDatum[] = [
    ...
    {
      category: "Bancarización",
      value: user.bancarization === "Alto" ? 90 : user.bancarization === "Medio" ? 60 : 30,
    },
  ...
  const spendingTrend: SpendingDatum[] = [
...
function renderDeviceLabel({ name, percent }: PieLabelRenderProps) {
...
function KPICard({ title, value, description, trend, change }: KPICardProps) {
